...
public class ClientControllerAdvisor {
    ...
    public ErrorResponse handleClientValidationIssues(Exception exception) {


        String errorMessage = "";
        if(exception instanceof ConstraintViolationException validationException) {
            log.info("handleClientValidationIssues getMessage() : [{}]", validationException.getMessage());
            log.warn("ConstraintViolationException", validationException);
            errorMessage = validationException.getConstraintViolations().toString();
        } else if( exception instanceof MethodArgumentNotValidException methodException) {
            log.info(" handleClientValidationIssues getMessage() : [{}]", methodException.getMessage());
            log.warn("MethodArgumentNotValidException", methodException);
            errorMessage = methodException.getMessage();
        }
